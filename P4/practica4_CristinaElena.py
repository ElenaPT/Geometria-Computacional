...
#hgt500 contiene todos los datos con presion fija 500hPa
...
#longitud en (-20,20)
i_ini = np.where(lons20 == 20.)[0][0] #(0, ini)
i_fin = np.where(lons20 == 340.)[0][0] + 1 # (fin, 360)
    ...
    for k in range(17):
        if k == k1 or k == k2:
            for j in range(j_ini,j_fin,1):
                ...
                for i in range (i_fin, len(lons20), 1):
                    di += 0.5*(hgt20[en20,k,j,i]-hgt[d,k,j,i])**2
...
print("Los cuatro dias analogos son:")
for i in range(4):
    print(dt_time[analogos[i]])
...
#Buscamos la posicion de los 1000hPa en el vector de presiones
p1000 = np.where(level == 1000)[0][0];
...
for j in range(j_ini,j_fin,1):
    for i in range (0,i_ini, 1):
        suma = 0
        for d in analogos:
            suma += air[d,p1000,j,i]
        suma = suma/4
        resta = np.abs(factor*suma - factor*air20[en20,p1000,j,i])
        media += resta
...
eam = media/((i_ini + len(lons20)-i_fin)*(j_fin-j_ini))
